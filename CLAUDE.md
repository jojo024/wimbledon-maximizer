...
- `static/index.html` — the single leaderboard page. Fetches `/api/combos`, renders each combo into a `.combo-strip` (horizontally-scrolling, cards sit still — no wandering/bobbing physics; that mechanic is exclusive to the Basket Builder arena), client-side sort (top/new), server-enforced one-vote ratings (`my_rating`), lazy-loaded comments with per-comment upvoting, top-comment snippet surfaced on the card, live-patched via `connectFeed()`. Combos flagged `honourable` are filtered out of the sortable strip and rendered into a separate strip instead (`cardHtml()` is shared between both; click/submit delegation lives on `document` rather than either strip so both work identically). Underneath, a Players section (`loadToday()`/`loadOverall()`, merged in from the former `static/players.html`) shows today's ranking and all-time standings (average distance since first submission, missed days counted as W$0, current streak), refetched on the `deal` WS event.
