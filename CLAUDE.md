...
8. **Broadcast Wordle** (`/wordle`) — another completely separate repo ([NickPoopy/broadcast-wordle](https://github.com/NickPoopy/broadcast-wordle)), but a built Vite/React/TypeScript app rather than zero-dependency static files. `WORDLE_DIR` (`WIM_WORDLE_DIR`, default `../broadcast-wordle/dist`) must point at its *built output*, produced with `VITE_BASE_PATH=/wordle/ npm run build` in that checkout (its `vite.config.ts` reads `VITE_BASE_PATH` specifically to support sub-path hosting like this) — not at the repo root, which has no servable `index.html` of its own. Same graceful disable-if-missing as Strawberry Rush, same no-import/no-build-from-here rule; the difference is updating it needs a rebuild (`git pull && npm ci && VITE_BASE_PATH=/wordle/ npm run build` in that checkout), not just a `git pull`, since `/wordle` serves the `dist/` snapshot rather than live source.
