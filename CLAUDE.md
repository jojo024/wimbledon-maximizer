...
6. **Pages** — `/`, `/builder`, `/tips`, `/meals`, `/play`, `/admin` serve files from `static/`; assets mount at `/static`.
7. **Companion games** (`/play`) — a hub, not a game itself. `GAMES` is a list of dicts (`slug`, `name`, `emoji`, `description`, `dir`, `repo`); at startup, each with an existing `dir` gets mounted read-only at `/play/<slug>` via `StaticFiles(..., html=True)`, and each missing one just logs a one-line NOTE instead of erroring. `GET /api/games` returns the same list with an `available` flag (live `dir.is_dir()` check) instead of a mount, and `static/play.html` fetches it to render a card per game — only the available ones get a working "Play" link. Every game is a **completely separate repo** — its own git history, README, release cadence — cloned as a sibling checkout one level up from this repo, never imported or built from here:
   - **Strawberry Rush** (`/play/strawberry-rush`, [jojo024/strawberry-rush](https://github.com/jojo024/strawberry-rush)) — zero-dependency static Canvas game. `GAME_DIR` (`WIM_GAME_DIR`, default `../strawberry-rush`) points straight at the checkout; updating it is a `git pull` there, no rebuild, no restart of this service.
   - **Broadcast Wordle** (`/play/wordle`, [NickPoopy/broadcast-wordle](https://github.com/NickPoopy/broadcast-wordle)) — a built Vite/React/TypeScript app. `WORDLE_DIR` (`WIM_WORDLE_DIR`, default `../broadcast-wordle/dist`) must point at its *built output* — `VITE_BASE_PATH=/play/wordle/ npm run build` in that checkout (its `vite.config.ts` reads `VITE_BASE_PATH` for exactly this sub-path case) — not the repo root, which has no servable `index.html`.
   - **BuggyRunner** (`/play/buggyrunner`, [NickPoopy/buggyrunner](https://github.com/NickPoopy/buggyrunner)) — also a built Vite app (Phaser), same shape as Wordle but its own env var name: `BASE_PATH=/play/buggyrunner/ npm run build` (not `VITE_BASE_PATH` — always check the target repo's own bundler config rather than assuming the name matches another game).
   Updating either Vite-built game needs a rebuild (`git pull && npm ci && <BASE_PATH_VAR>=/play/<slug>/ npm run build`), not just a `git pull`, since the mount serves the `dist/` snapshot rather than live source; Strawberry Rush needs neither a build nor a restart. See README § [Adding another game](README.md#adding-another-game) for the general recipe when integrating a new one.
...
- `static/play.html` — the games hub. Fetches `/api/games`, renders one `.game-card` per entry (emoji, name, description, a "Play" button linking to `/play/<slug>/` if `available`, a "Not available on this deployment" note otherwise). No client-side knowledge of which games exist or whether they're enabled — that's entirely server-derived from `GAMES`/directory checks in `main.py`, so adding a game never means touching this file.
