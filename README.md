...
- **Leaderboard** (`/`) — the whole competitive picture on one page. Glassmorphic combo cards sit in a horizontally-scrolling strip and update in real time over a WebSocket: new combos, ratings, and comments appear with a glow, no refresh. Sort by top rated or newest. The highest-upvoted comment on each combo is surfaced right on the card. Admin-curated, deliberately funky **honourable mention** combos sit right alongside everyone else's in the same strip — a trophy emoji next to the name is the only thing marking them out. Once there's enough to overflow, the strip slowly auto-scrolls on its own with a coverflow-style 3D tilt, pausing whenever you hover, read comments, or scroll it yourself. Underneath, the **Players** section ranks everyone by closeness to 30 Wimbledons: today's board plus an all-time leaderboard (average distance from 30 since your first submission — a skipped day counts as W$0 — and your current streak).
...
static/
  ...
  index.html         the leaderboard: combo cards (honourable mentions included,
                      marked with a trophy emoji) in a horizontally-scrolling strip,
                      rating, comments, comment upvotes, plus the Players ranking
                      (today + all-time standings, avg distance, streaks)
...
| `/api/admin/combos` | POST | `X-Admin-Key` | Create an "honourable mention" combo directly (still must total exactly 3000 cents) — sorts and rates in the leaderboard strip like any other combo, flagged `honourable` so the UI can mark it with a trophy emoji |
...
- **Reactions carry a signal; not all of them count.** Tips have six independent reaction toggles, but only up/down feed the sort `score` — fire/heart/laugh/cry are expressive, not competitive.
- **Honourable mentions are real combos, not a side exhibit.** They're valid, exactly-30-Wimbledon combos that an admin created directly instead of a player submitting them — the `honourable` flag exists to mark that provenance, not to wall them off. They sort, rate, and comment exactly like anything else in the strip; a trophy emoji next to the name is the only thing that says "admin-curated." An earlier version gave them their own section, excluded from the ranking entirely — this was a deliberate simplification once that separation turned out to just be visual clutter for a distinction nobody needed enforced.
